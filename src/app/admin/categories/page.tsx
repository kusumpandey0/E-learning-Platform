      ...
      <div className="overflow-x-auto">
        <div className="min-w-full inline-block align-middle">
          <div className="relative text-gray-500 focus-within:text-gray-900 mb-4">
            ...
            <input type="text" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search for company" />
          </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full rounded-xl">
              <thead>
                <tr className="bg-gray-50">
                  <th className="p-5 text-left text-sm font-semibold text-gray-900 capitalize">Company</th>
                  <th className="p-5 text-left text-sm font-semibold text-gray-900 capitalize">User ID</th>
                  <th className="p-5 text-left text-sm font-semibold text-gray-900 capitalize">Type</th>
                  <th className="p-5 text-left text-sm font-semibold text-gray-900 capitalize">Industry Type</th>
                  <th className="p-5 text-left text-sm font-semibold text-gray-900 capitalize">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-300">
                {categories.length > 0 ? (
                  categories.map((category: ICategory) => (
                    <tr key={category._id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                      <td className="p-5 whitespace-nowrap text-sm font-medium text-gray-900">{category._id}</td>
                      <td className="p-5 whitespace-nowrap text-sm font-medium text-gray-900">{category.name}</td>
                      <td className="p-5 whitespace-nowrap text-sm font-medium text-gray-900">{category.description || "No description"}</td>
                      <td className="p-5 whitespace-nowrap text-sm font-medium text-gray-900">{category.createdAt ? new Date(category.createdAt).toLocaleDateString() : "N/A"}</td>
                      <td className="p-5">
                        <button className="px-4 py-2 bg-indigo-500 text-white rounded-full">Edit</button>
                        <button className="ml-2 px-4 py-2 bg-red-500 text-white rounded-full">Delete</button>
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={5} className="text-center p-5">No categories found.</td>
                  </tr>
                )}
              </tbody>
            </table>
